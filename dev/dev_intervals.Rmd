...
.nearestWhite <- function(note){
  ...
  withFlats <- ALPHABET[which(ALPHABET$HAS_FL),"LETTER"]
  withSharps <- ALPHABET[which(ALPHABET$HAS_SH),"LETTER"]
  # Get note letter
  ...
  # Get neighbouring notes ready
  above <- note %INC% "#"
  below <- note %INC% "b"
  # Find nearest white key
  # Do we care to && above %in% withFlats?
  if(isFlat){ 
    list(above, -1)
  } else if(isSharp){
    list(below, 1)
